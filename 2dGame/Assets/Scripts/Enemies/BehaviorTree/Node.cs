...
public abstract class Node
{
    ...
    public float NodeWeight
    {
        get
        {
            if (_lerpWeight)
            {
                NodeWeight = _nodeWeight - (((1 - _baseWeight) / _decrementTime) * (Time.time - _updateTime));
                _updateTime = Time.time;
            }
            else
            {
                NodeWeight = Mathf.Ceil(1 - ((Time.time - _updateTime) / _decrementTime));
            }
            return _nodeWeight;
        }
    ...
    public Node(float actionWeight = 0, float baseWeight = 0, float decrementTime = 1, bool lerpWeight = true)
    {
